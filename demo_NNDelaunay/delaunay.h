...
*/
    ...
    */
    static int in_circle( point2d_t *pt0, point2d_t *pt1, point2d_t *pt2, point2d_t *p )
    {
            ...
            float	p0p_x, p0p_y, p1p_x, p1p_y, p2p_x, p2p_y, p0p, p1p, p2p, res;
            mat3_t	m;
    ...
    */
    ...
    /*
    */
    delaunay2d_t* delaunay2d_from(Point *points, unsigned int num_points) {
            delaunay2d_t*	res	= NULL;
            delaunay_t	del;
            unsigned int	i, j, fbuff_size = 0;
            unsigned int*	faces	= NULL;

                    ////mexEvalString("disp('w--------2')");
 ...
 //           std::sort(del.points,del.points+num_points, cmp_points);
        ////mexEvalString("disp('gg--------2')");

            if( num_points >= 3 ) {
                        ////mexEvalString("disp('gkg--------2')");

                    del_divide_and_conquer( &del, 0, num_points - 1 );
        ////mexEvalString("disp('gyg--------2')");

                    del_build_faces( &del );
        ////mexEvalString("disp('grg--------2')");
        ...
        ////mexEvalString("disp('geg--------2')");
                ...
                ////mexEvalString("disp('ggada--------2')");

            res		= (delaunay2d_t*)malloc(sizeof(delaunay2d_t));
            assert( NULL != res );
            res->num_points	= num_points;
            res->points	= (Point*)malloc(sizeof(Point) * num_points);
            assert( NULL != res->points );
            memcpy(res->points, points, sizeof(Point) * num_points);
            res->num_faces	= del.num_faces;
            res->faces	= faces;

                    ////mexEvalString("disp('dd--------2')");
    ...
    tri_delaunay2d_t* tri_delaunay2d_from(delaunay2d_t* del) {
            ...
            tdel->points		= (Point*)malloc(sizeof(Point) * del->num_points);
            assert( NULL != tdel->points );
            memcpy(tdel->points, del->points, sizeof(Point) * del->num_points);
...
public:
    ...
    void reserve(unsigned int data_){
        points.reserve(data_);
    }
  ...
  float nearZ(int px,int py, Point &p0, Point &p1, Point &p2){

      float x=(p0.x - px) * (p0.x - px) + (p0.y - py) * (p0.y - py);
      float y=(p1.x - px) * (p1.x - px) + (p1.y - py) * (p1.y - py);
      float z=(p2.x - px) * (p2.x - px) + (p2.y - py) * (p2.y - py);
...
//           return p2.z;
//       }
    return  x < y ? (x < z ? p0.z : p2.z) : (y < z ? p1.z : p2.z);
    ...
    void triangulate()
    {
        ...
        if( points.size() >= 3 )
        {
        ////mexEvalString("disp('--ee')");

            delaunay2d_t*	res = delaunay2d_from(points.data(), points.size());
                    ////mexEvalString("disp('--ee3')");

            tri_delaunay2d_t*	tdel	= tri_delaunay2d_from(res);
        ////mexEvalString("disp('--wee')");

            triangles.reserve(tdel->num_triangles*3);
          //  tripoints.reserve(tdel->num_triangles*3);
        ////mexEvalString("disp('-ww-ee')");
            ...
            for(int i = 0; i < tdel->num_triangles; i++ )
            {
                for( int j = 0; j < 3; j++ )
                {
                    index=tdel->tris[i * 3 + j];
                    triangles.push_back(index);
                   // tripoints.push_back(points[index]);
                }
            }
        ////mexEvalString("disp('-wwqr-ee')");

            tri_delaunay2d_release(tdel);
                    ////mexEvalString("disp('--eetrt')");

            delaunay2d_release(res);
                                ////mexEvalString("disp('--eeqeqwetrt')");
        ...
        void line(  int x0,   int y0,   int x1,   int y1,int x2,int y2,double *y,int dim,float val){
     ...
     void line(  int x0,   int y0,   int x1,   int y1,double *y,int dim,float val){
    ...
    void line(  int x0,   int y0,   int x1,   int y1,int x2,int y2,double *y,int dim,Point &p1,Point &p2,Point &p3){
        ...
        while(true){
            ...
            if (e2 > -dy){
                err = err - dy;
                x0 = x0 + sx;
                line(x0,y0,x2,y2,y,dim,p1,p2,p3);
...
//             }
     ...
     void line(  int x0,   int y0,   int x1,   int y1,double *y,int dim,Point &p1,Point &p2,Point &p3){
        ...
        while(true){
                           y[x0*dim  + y0]=nearZ(x0,y0,p1,p2,p3);
            if ((x0 == x1) && (y0 == y1)){
                //  unprotected_setOccupied(x0,y0);
                //exit loop
                return;
        ...
        void line(  int x0,   int y0,   int x1,   int y1,std::vector<std::pair<int,int> >& out){
   ...
   void mapEachTriangle(double *y,unsigned int dim,float init,  float total){
       ...
       //outline.reserve(10);
        ...
        int startv=floor((init/total)*((double)triangles.size()/3.0))*3.0;
        int   endv=floor(((init+1.0)/total)*((double)triangles.size()/3.0))*3.0;
      
       /*if(quarter==0){
           startv=0;
           endv=floor(0.25*((double)triangles.size()/3.0))*3.0;
       }else        if(quarter==1){
           startv=floor(0.25*((double)triangles.size()/3.0))*3.0;
           endv=floor(0.5*((double)triangles.size()/3.0))*3.0;
       }else        if(quarter==2){
           startv=floor(0.5*((double)triangles.size()/3.0))*3.0;
           endv=floor(0.75*((double)triangles.size()/3.0))*3.0;
       }else        if(quarter==3){
           startv=floor(0.75*((double)triangles.size()/3.0))*3.0;
           endv=triangles.size();
       }*/
      // int startv=quarter/4.0*((double)triangles.size()/3.0);
      // int endv=(quarter+1.0/4.0)
       int p3x=0;
       int p3y=0;
       for( int i=startv;i<endv;i=i+3){
       
         //   out.clear();
         //   outline.clear();
            Point &p1=points[triangles[i]];
            ...
            line( round(p1.x),round(p1.y),round(p2.x),round(p2.y),p3x,p3y,y,dim,p1,p2,p3);
            
            //line( round(p1.x),round(p1.y),round(p2.x),round(p2.y),p3x,p3y,y,dim,i/3);
//             line(round(p1.x),round(p1.y),round(p2.x),round(p2.y),out);
//             
//             for(unsigned int g=0;g<out.size();g++){
//                 line(out[g].first,out[g].second,p3x,p3y,outline);
//             }
//             
//             for(unsigned int g=0;g<outline.size();g++){
//                 y[outline[g].first*(int)dim  + outline[g].second]=nearZ(outline[g].first,outline[g].second,p1,p2,p3);
//             }
